...
PREV_EXEC=""
if [[ -f "$UNIT_DIR/grok-build-livepatch.service" ]]; then
  PREV_EXEC=$(grep -E '^ExecStart=' "$UNIT_DIR/grok-build-livepatch.service" | head -1 || true)
fi
...
NEW_EXEC=$(grep -E '^ExecStart=' "$UNIT_DIR/grok-build-livepatch.service" | head -1 || true)
...
echo "Tip: re-run this script after pulling so the timer tracks this checkout (not a stale plugin copy)."
