...
|---|---|
| `scout`, `connector`, `the-planner` | `xask --provider cursor --model-id kimi-k3-max --gs`. Never `--spark` on this pin. Never default `cdx`. Never Claude. Never `auto`. |
