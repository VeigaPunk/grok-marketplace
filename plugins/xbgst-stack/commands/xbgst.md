...
- **Crossbreed / xask-first.** Spawn native Grok subagent types from this plugin (`scout`, `reviewer`, `labrat`, `executor`, `connector`, `distiller`, `simplifier`, `the-revenger`, `sentinel`, `critic`, `mutation-tester`, `scribe`, `the-planner`, `the-janitor`, …). Every consult-role dispatch is a **runner** whose FIRST tool-using step is PATH `xask` (never spawn type `xask`; never `xask-l3` as FIRST). ChatGPT xask defaults to **sekhmet / `codex-titanium` / `service_tier=fast`**. Copy-paste: `xask --spark --gs --service-tier fast cdx '<q>'`. Quote `<raw_output>` from **result.json stdout** (never the sekhmet CLI envelope; see `references/xbreed-shared.md` Extract). On failure `BLOCKED: xask [reason]` and continue. **No Claude TeamCreate.**
