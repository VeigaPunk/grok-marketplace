...
## Optional substrate routing: L2-loop vs L2-fsd vs L2-select vs L3
...
| Cursor-agent write+shell FSD subprocess | **optional xbgst-cursor L2-fsd** via `scripts/cursor-agent-l2.sh` / `/xbgst-cursor` | workspace `/home/vgpnk/Projects/xbgst/xbgst-cursor`; print-argv default; exec only `XBGST_CURSOR_EXEC=1`; never `--mode ask`; never judge, Pareto, APPROVED, or ship; `xbgst-cursor-agent-surface` stays trigger/forward |
...
Cursor-agent **xbgst-cursor L2-fsd** is an optional sibling, not a PrimeAgent replacement. L1 assigns a named `gx-*` route owner and documents the same seven-key envelope; `scripts/cursor-agent-l2.sh` prints argv by default and does not execute that envelope. Exec only if `XBGST_CURSOR_EXEC=1`. Workspace is `/home/vgpnk/Projects/xbgst/xbgst-cursor`. Catalog pin is `cursor-agent --model <cli-id>` or `XBGST_CURSOR_MODEL` — not xask `--model-id`. Refuse `--mode ask`, `--force`/`--yolo`, `--plugin-dir`, and argv0=`agent`. PATH `xask --provider cursor` is agentic `-p --trust` (default `kimi-k3-max`). Surface `xbgst-cursor-agent-surface` stays trigger/forward. Absence falls back to native `gx-*` without blocking L1.
...
3. Freeze the roster, then `spawn_subagent` the useful named specialists concurrently up to the host-governed ceiling (certified at 64), each with Godspeed injected and a DESPAWN closer. Do **not** introduce a smaller package-level cap (no 16/wave, no 4-slot default, no “floor 8”). **BANNED wave sizes (vanilla fingerprints only): 4 and 5.** Do **not** ban 1–7 as a range — that trains L1 to spawn exactly 8. Defaulting to the minimum legal wave is forbidden; fill toward 64 when the roster has work. Never pad with `general-purpose` or `explore`. Overflow demand routes to spark substrates at `-j 64`, launched in the SAME turn. Never split a wave across turns; never trickle-dispatch 1–2 when more roster rows exist. Resilience: failed spawns are retried or abandoned immediately, never awaited indefinitely. **Always include connector** in every PROPOSE after Round 0. Then **wait for DESPAWN** from the roster — one `get_command_or_subagent_output` on all ids — not a user-visible recap per child.
4. Distiller after the DESPAWN set. Then Pareto filter: evidence gate first (drop moves missing required `evidence:`); then accept remaining moves that improve ≥1 axis and regress none.
...
**Round phases:** PROPOSE (parallel `spawn_subagent` wave, must contain connector) → wait DESPAWN set → CROSS-CRITIQUE → distiller `SYNTHESIS_READY` → PARETO FILTER (judge) → COMPILE (scribe concurrent with Pareto) → **if milestone shippable: APPROVED + commit + push `main` (local-first)**. If any axis improved, dispatch the next PROPOSE immediately inside the same activation — do not pause, do not emit a round-boundary prompt, do not ask. The entire loop runs to completion in one response stream. Exit only with final DRAFT + AXES FINAL STATE.

**Autonomous iteration:** One activation = full multi-round loop. The judge orchestrates every round internally until the frontier is reached (zero axis improvement vs previous round) or the hard 6-round cap. Emit only the final DRAFT + AXES FINAL STATE. Never emit "Round N ready", "continuing", "will run", a per-subagent recap, or any intermediate prompt that waits for user input. No input from the user is necessary after the initial trigger. User interrupt is the sole external control. Usage telemetry, if any, writes files only — it is not the round loop.
